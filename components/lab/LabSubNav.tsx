...
export default function LabSubNav() {
  ...
  return (
    <div className="border-b border-lime-900/30 bg-gray-950/60 backdrop-blur-sm">
          ...
          {items.map(({ href, label, Icon, exact }) => {
            ...
            return (
              <Link
                key={href}
                href={href}
                className={`flex items-center gap-2 px-3 py-2 text-sm rounded-sm transition-all duration-300 whitespace-nowrap ${
                  active
                    ? "bg-lime-900/30 text-lime-300 border border-lime-700/50"
